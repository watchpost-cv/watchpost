...
func (s *Store) migrate(ctx context.Context) error {
	...
	if version == 11 {
		if _, err = tx.ExecContext(ctx, `CREATE TABLE bootstrap_tokens(token_hash BLOB PRIMARY KEY, expires_at TEXT NOT NULL, consumed_at TEXT)`); err != nil {
	...
	if version == 12 {
		statements := []string{
			`ALTER TABLE collector_keys ADD COLUMN pending_secret_hash BLOB`,
			`ALTER TABLE collector_keys ADD COLUMN pending_expires_at TEXT`,
		}
		for _, statement := range statements {
			if _, err = tx.ExecContext(ctx, statement); err != nil {
	...
	if version == 13 {
		statements := []string{
			`ALTER TABLE device_profiles ADD COLUMN encrypted_auth BLOB`,
			`ALTER TABLE device_profiles ADD COLUMN encrypted_privacy BLOB`,
			`ALTER TABLE device_profiles ADD COLUMN interval_seconds INTEGER NOT NULL DEFAULT 0`,
			`ALTER TABLE device_profiles ADD COLUMN next_run_at TEXT`,
			`ALTER TABLE device_profiles ADD COLUMN enabled INTEGER NOT NULL DEFAULT 0`,
			`CREATE INDEX device_profiles_due ON device_profiles(enabled,next_run_at)`,
		}
		for _, statement := range statements {
			if _, err = tx.ExecContext(ctx, statement); err != nil {
	...
	if version == 14 {
		if _, err = tx.ExecContext(ctx, `CREATE TABLE launcher_instances(id TEXT PRIMARY KEY,position INTEGER NOT NULL UNIQUE,name TEXT NOT NULL,domain TEXT NOT NULL,port INTEGER CHECK(port BETWEEN 1 AND 65535))`); err != nil {
	...
	if version == 18 {
		if _, err = tx.ExecContext(ctx, `CREATE TABLE cluster_outbound_joins(id TEXT PRIMARY KEY,remote_url TEXT NOT NULL,request_id TEXT NOT NULL,request_secret TEXT NOT NULL,local_inbound_credential TEXT NOT NULL,state TEXT NOT NULL CHECK(state IN ('pending','approved','rejected','expired','failed')),created_at TEXT NOT NULL,updated_at TEXT NOT NULL,last_error TEXT NOT NULL DEFAULT '')`); err != nil {
	...
	if version == 20 {
		var exists int
		if err = tx.QueryRowContext(ctx, `SELECT COUNT(*) FROM pragma_table_info('collector_keys') WHERE name='last_batch_id'`).Scan(&exists); err != nil {
			return fmt.Errorf("migration 21: %w", err)
		}
		if exists == 0 {
			if _, err = tx.ExecContext(ctx, `ALTER TABLE collector_keys ADD COLUMN last_batch_id TEXT`); err != nil {
				return fmt.Errorf("migration 21: %w", err)
			}
		}
