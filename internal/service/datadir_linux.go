...
func inspectDataDir(path string) (dataDirPlan, error) {
	...
	if info.uid != uid {
		closeFdSeam(fd)
		return dataDirPlan{}, fmt.Errorf("data directory %q already exists and is owned by UID %d; the watchpost service requires it to be owned by %s:%s with mode 0700. Move existing data under %s or re-home it; the installer will not adopt an existing directory", path, info.uid, ServiceUser, ServiceGroup, DefaultDataDir)
	}
