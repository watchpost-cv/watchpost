...
func TestInstallStateMatrix(t *testing.T) {
	...
	for _, tc := range matrix {
		t.Run(tc.name, func(t *testing.T) {
			...
			os.WriteFile(exe, []byte("#!/bin/sh\nexit 0\n"), 0o755)
			if tc.accepted {
				// The reinstall changes the listen, so activation path runs;
				// script the forward steps. enable/restart fail to force rollback.
...
func TestInstallStateQueryFailureAborts(t *testing.T) {
	...
	installManagedUnit(t)
	beforeBin, _ := os.ReadFile(BinaryPath)
	// is-enabled query failure must abort before mutation.
...
func TestDataDirNewLeafAdopted(t *testing.T) {
	r := newStrictService(t)
	dir := t.TempDir()
	newData := filepath.Join(dir, "leaf", "watchpost-data")
	exe := filepath.Join(t.TempDir(), "wp")
	os.WriteFile(exe, []byte("#!/bin/sh\nexit 0\n"), 0o755)
	oldChown := chownData
	chowned := ""
