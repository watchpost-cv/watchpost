# Watchpost automation

Example session workflow:

```sh
printf '%s\n' '{"email":"admin@example.invalid","password":"REDACTED"}' > /tmp/watchpost-login.json
chmod 600 /tmp/watchpost-login.json
# Choose the login command from the generated matrix, then persist the returned session:
watchpost <login-resource> <login-verb> --input /tmp/watchpost-login.json --session-file "$HOME/.config/watchpost/session.json" --json
```
