(()=>{
  ...
  q("[data-add]").onclick=()=>q("dialog").showModal();
  q("form").onsubmit=async event=>{if(event.submitter?.value!=="save")return;event.preventDefault();await api("/api/v1/users",{method:"POST",body:JSON.stringify(Object.fromEntries(new FormData(event.currentTarget)))});q("dialog").close();load()};
