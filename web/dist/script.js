...
function route() {
  ...
  if (current === "fleet") renderCluster();
...
async function renderCluster(){const identityNode=$("#cluster-identity"),pendingNode=$("#cluster-pending"),membersNode=$("#cluster-members");if(!identityNode)return;identityNode.innerHTML=stateBox("Loading cluster","Reading local identity and membership.","loading");try{const [identity,pending,members,status,outbound]=await Promise.all([request("/api/v1/cluster/identity"),request("/api/v1/cluster/joins"),request("/api/v1/cluster/members"),request("/api/v1/cluster/status"),request("/api/v1/cluster/outbound-joins")]);const i=identity.identity;identityNode.innerHTML=`<p class="eyebrow">This Watchpost</p><h2>${escapeHTML(i.display_name||i.node_id)}</h2><p><code>${escapeHTML(i.node_id)}</code> · fingerprint <code>${escapeHTML(identity.fingerprint)}</code></p><p>${escapeHTML(i.public_endpoint||"Public HTTPS endpoint not configured")}</p>`;const outboundNode=$("#cluster-outbound");outboundNode.innerHTML=`<p class="eyebrow">Outbound joins</p><h2>${outbound.length?`${outbound.length} join request${outbound.length===1?"":"s"}`:"No outbound joins"}</h2>${outbound.map(j=>`<article class="evidence-row"><div><h3>${escapeHTML(j.remote_url)}</h3><p>${escapeHTML(j.state)} · request <code>${escapeHTML(j.request_id)}</code></p></div><div class="actions">${j.state==="pending"?`<button class="quiet-button" data-cluster-collect="${escapeHTML(j.id)}">Collect approval</button>`:""}</div></article>`).join("")}`;$$('[data-cluster-collect]',outboundNode).forEach(b=>b.onclick=()=>clusterCollect(b.dataset.clusterCollect));pendingNode.innerHTML=`<p class="eyebrow">Pending joins</p><h2>${pending.length?`${pending.length} awaiting review`:"No pending joins"}</h2>${pending.map(j=>`<article class="evidence-row"><div><h3>${escapeHTML(j.display_name||j.node_id)}</h3><p>${escapeHTML(j.public_endpoint)} · <code>${escapeHTML(j.fingerprint)}</code></p></div><div class="actions"><button data-cluster-approve="${escapeHTML(j.id)}">Approve</button><button class="quiet-button" data-cluster-reject="${escapeHTML(j.id)}">Reject</button></div></article>`).join("")}`;const health=new Map(status.results.map(r=>[r.node_id,r]));membersNode.innerHTML=`<p class="eyebrow">Members</p><h2>${members.length?`${members.length} paired node${members.length===1?"":"s"}`:"Standalone"}</h2>${members.map(m=>`<article class="evidence-row"><div><h3>${escapeHTML(m.display_name||m.node_id)}</h3><p>${escapeHTML(m.health)} · ${escapeHTML(m.product_version)} · ${escapeHTML(m.public_endpoint)}</p></div><div class="actions"><button class="quiet-button" data-cluster-disable="${escapeHTML(m.node_id)}">${m.state==="disabled"?"Enable":"Disable"}</button><button class="quiet-button" data-cluster-revoke="${escapeHTML(m.node_id)}">Revoke</button></div></article>`).join("")}`;$$('[data-cluster-approve]',pendingNode).forEach(b=>b.onclick=()=>clusterDecision(b.dataset.clusterApprove,"approve"));$$('[data-cluster-reject]',pendingNode).forEach(b=>b.onclick=()=>clusterDecision(b.dataset.clusterReject,"reject"));$$('[data-cluster-disable]',membersNode).forEach(b=>b.onclick=()=>clusterMemberToggle(b.dataset.clusterDisable,members.find(m=>m.node_id===b.dataset.clusterDisable)?.state));$$('[data-cluster-revoke]',membersNode).forEach(b=>b.onclick=()=>clusterRevoke(b.dataset.clusterRevoke));void health;await renderPropagation()}catch(error){identityNode.innerHTML=stateBox("Cluster unavailable",error.message,"error")}}

async function clusterCollect(id){try{const result=await request(`/api/v1/cluster/outbound-joins/${encodeURIComponent(id)}/collect`,{method:"POST",headers:{"X-Watchpost-CSRF":state.csrf}});await renderCluster();showMessage(result.state==="approved"?"Watchpost joined the cluster.":`Join is ${result.state}.`)}catch(error){showMessage(error.message,"error")}}
$("#cluster-join").addEventListener("submit",async event=>{event.preventDefault();const form=event.currentTarget,values=formJSON(form);setBusy(form,true);try{const result=await request("/api/v1/cluster/outbound-joins",{method:"POST",headers:{"X-Watchpost-CSRF":state.csrf},body:JSON.stringify(values)});form.reset();await renderCluster();showMessage(`Join request ${result.request_id} submitted. Approve it on the remote Watchpost, then collect approval here.`)}catch(error){showMessage(error.message,"error")}finally{setBusy(form,false)}});
async function clusterDecision(id,action){try{await request(`/api/v1/cluster/joins/${encodeURIComponent(id)}/${action}`,{method:"POST",headers:{"X-Watchpost-CSRF":state.csrf}});await renderCluster();showMessage(`Cluster join ${action}d.`)}catch(error){showMessage(error.message,"error")}}
async function clusterMemberToggle(id,stateValue){const action=stateValue==="disabled"?"enable":"disable";try{await request(`/api/v1/cluster/members/${encodeURIComponent(id)}/${action}`,{method:"POST",headers:{"X-Watchpost-CSRF":state.csrf}});await renderCluster()}catch(error){showMessage(error.message,"error")}}
...
async function renderPropagation(){const root=$("#cluster-propagation");if(!root)return;try{const [kinds,profiles,history]=await Promise.all([request("/api/v1/cluster/propagation/kinds"),request("/api/v1/cluster/propagation/profiles"),request("/api/v1/cluster/propagation/history")]);root.innerHTML=`<p class="eyebrow">Configuration propagation</p><h2>Propagate configuration</h2><p>Preview exact changes before applying. Runtime data and databases are never replicated.</p><form id="propagation-run"><div class="check-list">${kinds.map(k=>`<label><input type="checkbox" name="kind" value="${escapeHTML(k.kind)}" checked> ${escapeHTML(k.label||k.kind)}</label>`).join("")}</div><div class="actions"><button name="action" value="preview">Preview all members</button><button name="action" value="apply">Apply to all members</button></div></form><pre id="propagation-result" class="result-panel"></pre><h3>Reconciliation profile</h3><form id="propagation-profile"><label>ID<input name="id" required placeholder="baseline"></label><label>Name<input name="name" required placeholder="Baseline configuration"></label><label>Mode<select name="mode"><option value="notify-only">Notify only</option><option value="approval-required">Approval required</option><option value="automatic">Automatic</option></select></label><label>Schedule<input name="schedule" placeholder="15m (blank = manual)"></label><label>Maintenance window<input name="window" placeholder="23:00-02:00"></label><button>Save profile</button><button type="button" id="propagation-run-due" class="quiet-button">Run due profiles now</button></form><h3>Profiles</h3><div class="evidence-list">${profiles.map(p=>`<article class="evidence-row"><div><h3>${escapeHTML(p.name)}</h3><p>${escapeHTML(p.mode)} · ${escapeHTML(p.schedule||"manual")} · ${p.last_run_at?`last ${escapeHTML(p.last_run_at)}`:"never run"}</p></div><button class="quiet-button" data-prop-delete="${escapeHTML(p.id)}">Delete</button></article>`).join("")||"<p>No saved profiles.</p>"}</div><h3>Propagation history</h3><div class="evidence-list">${history.slice(0,10).map(h=>`<article class="evidence-row"><div><h3>${escapeHTML(h.status)}</h3><p>${escapeHTML(h.plan_id)} · applied ${h.applied} · failed ${h.failed} · rolled back ${h.rolled_back}</p></div></article>`).join("")||"<p>No propagation history.</p>"}</div>`;$("#propagation-run").onsubmit=async e=>{e.preventDefault();const fd=new FormData(e.currentTarget),k=[...fd.getAll("kind")],action=e.submitter?.value||"preview",body={kinds:k,selector:{all:true},dry_run:action==="preview"};if(action==="apply"&&!confirm("Apply the previewed configuration to all active cluster members?"))return;const out=await request("/api/v1/cluster/propagation/propagate",{method:"POST",headers:{"X-Watchpost-CSRF":state.csrf},body:JSON.stringify(body)});$("#propagation-result").textContent=JSON.stringify(out,null,2);await renderPropagation()};$("#propagation-profile").onsubmit=async e=>{e.preventDefault();const f=new FormData(e.currentTarget),k=[...document.querySelectorAll('#propagation-run input[name="kind"]:checked')].map(v=>v.value),id=String(f.get("id"));await request(`/api/v1/cluster/propagation/profiles/${encodeURIComponent(id)}`,{method:"PUT",headers:{"X-Watchpost-CSRF":state.csrf},body:JSON.stringify({id,name:f.get("name"),selector:{all:true},kinds:k,mode:f.get("mode"),schedule:f.get("schedule"),maintenance_window:f.get("window"),enabled:true})});showMessage("Propagation profile saved.");await renderPropagation()};$("#propagation-run-due").onclick=async()=>{const out=await request("/api/v1/cluster/propagation/profiles/run-due",{method:"POST",headers:{"X-Watchpost-CSRF":state.csrf},body:"{}"});$("#propagation-result").textContent=JSON.stringify(out,null,2);await renderPropagation()};$$('[data-prop-delete]',root).forEach(b=>b.onclick=async()=>{await request(`/api/v1/cluster/propagation/profiles/${encodeURIComponent(b.dataset.propDelete)}`,{method:"DELETE",headers:{"X-Watchpost-CSRF":state.csrf}});await renderPropagation()})}catch(error){root.innerHTML=stateBox("Propagation unavailable",error.message,"error")}}
