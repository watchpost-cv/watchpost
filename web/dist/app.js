...
$("#collector").addEventListener("submit", async event => { event.preventDefault(); const form = event.currentTarget, values = formJSON(form), output = $("#collector-result"); output.hidden = false; output.innerHTML = stateBox("Creating pairing token", "Preparing a short-lived one-use command.", "loading"); setBusy(form, true); try { const result = await request(`/api/v1/posts/${encodeURIComponent(values.post)}/pairing-tokens`, { method: "POST", headers: { "X-Watchpost-CSRF": state.csrf }, body: JSON.stringify({}) }); const server = location.origin, command = `./watchpost collector pair --server ${server} --token ${result.token}`; output.innerHTML = `<h2>Pair within 10 minutes</h2><p>Run this on the monitored Linux machine. The token can be used once and is exchanged for a post-scoped credential stored with private permissions.</p><pre class="secret-block"><code>${escapeHTML(command)}</code></pre><button id="copy-collector" class="quiet-button" type="button">Copy command</button><p class="callout">Remote pairing requires HTTPS. Plain HTTP is accepted only on loopback for local development.</p>`; $("#copy-collector").onclick = async () => { await navigator.clipboard.writeText(command); showMessage("Pairing command copied."); }; } catch (error) { output.innerHTML = stateBox("Pairing token failed", error.message, error.message.includes("permission") ? "permission" : "error"); } finally { setBusy(form, false); } });
