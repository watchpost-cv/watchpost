...
// Default Watchpost host and port used by the shared project configuration
// pattern. Watchpost's default listener is 127.0.0.1:7334; both remain
// configurable.
const (
...
func resolveHostPort(hostFlag, portFlag string, hostSet, portSet bool) (host, port string, err error) {
	...
	} else if v, ok := os.LookupEnv("WATCHPOST_HOST"); ok {
		host = strings.TrimSpace(v)
		if host == "" {
			return "", "", errors.New("WATCHPOST_HOST is set but empty")
		}
	}
	port = defaultPort
	if portSet {
	...
	} else if v, ok := os.LookupEnv("WATCHPOST_PORT"); ok {
		if err := validatePort(v); err != nil {
			return "", "", fmt.Errorf("WATCHPOST_PORT: %w", err)
		}
...
//   - explicit --host and/or --port override the legacy WATCHPOST_LISTEN variable;
//   - with only environment variables, legacy WATCHPOST_LISTEN conflicts with
//     WATCHPOST_HOST/WATCHPOST_PORT rather than silently picking one;
//   - otherwise WATCHPOST_LISTEN is used, then host/port defaults.
//
...
func resolveListener(hostFlag, portFlag, listenFlag string, hostSet, portSet, listenSet bool) (string, error) {
	...
	// Explicit --host/--port override the legacy WATCHPOST_LISTEN variable.
	if hostSet || portSet {
	...
	// Only environment variables are involved: legacy WATCHPOST_LISTEN versus
	// the new WATCHPOST_HOST/WATCHPOST_PORT forms must not silently pick one.
	if v, ok := os.LookupEnv("WATCHPOST_LISTEN"); ok {
		if _, hasHost := os.LookupEnv("WATCHPOST_HOST"); hasHost {
			return "", errors.New("WATCHPOST_LISTEN cannot be combined with WATCHPOST_HOST")
		}
		if _, hasPort := os.LookupEnv("WATCHPOST_PORT"); hasPort {
			return "", errors.New("WATCHPOST_LISTEN cannot be combined with WATCHPOST_PORT")
		}
		if strings.TrimSpace(v) == "" {
			return "", errors.New("WATCHPOST_LISTEN is set but empty")
		}
		return v, nil
	}
	host, port, err := resolveHostPort(hostFlag, portFlag, hostSet, portSet)
...
// host/port listener form (CLI flags or WATCHPOST_HOST/WATCHPOST_PORT
...
func listenerOverrideSelected(fs *flag.FlagSet) bool {
	...
	if _, ok := os.LookupEnv("WATCHPOST_HOST"); ok {
		return true
	}
	if _, ok := os.LookupEnv("WATCHPOST_PORT"); ok {
		return true
	}
	return false
}
