...
if [ "$system" -eq 1 ]; then [ "$(id -u)" -eq 0 ] || { echo '--system requires root' >&2; exit 1; }; destination=/usr/local/bin
...
name="watchpost-${version}-${os}-${arch}"; tmp=$(mktemp -d); trap 'rm -rf "$tmp"' EXIT HUP INT TERM
